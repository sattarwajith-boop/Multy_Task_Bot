...
apply_config()
install()
# Python 3.10+ and recent uvloop versions do not implicitly create a current
# event loop. Pyrogram's Client constructor expects one to already exist.
...
# Koyeb/host environment values must win over the local template.  The
# upstream project used override=True, which can silently replace deployed
# Secrets with blank or stale values from config.env.
...
def _is_unset(value):
    return not value or str(value).strip().upper() in {'CHANGE_ME', 'YOUR_VALUE'}


BOT_TOKEN = environ.get('BOT_TOKEN', '')
if _is_unset(BOT_TOKEN) or ':' not in BOT_TOKEN:
    log_error("BOT_TOKEN is missing or invalid! Exiting now")
    exit(1)

bot_id = BOT_TOKEN.split(':', 1)[0]

DATABASE_URL = environ.get('DATABASE_URL', '')
if _is_unset(DATABASE_URL):
    DATABASE_URL = ''
...
if DATABASE_URL:
    ...
    if old_config and old_config != current_config:
        ...
        for key, value in config_dict.items():
            if key not in SENSITIVE_CONFIG_KEYS and not environ.get(key):
...
if _is_unset(OWNER_ID) or not OWNER_ID.lstrip('-').isdigit():
    log_error("OWNER_ID is missing or invalid! Exiting now")
    exit(1)
else:
    OWNER_ID = int(OWNER_ID)

TELEGRAM_API = environ.get('TELEGRAM_API', '')
if _is_unset(TELEGRAM_API) or not TELEGRAM_API.isdigit():
    log_error("TELEGRAM_API is missing or invalid! Exiting now")
    exit(1)
else:
    TELEGRAM_API = int(TELEGRAM_API)

TELEGRAM_HASH = environ.get('TELEGRAM_HASH', '')
if _is_unset(TELEGRAM_HASH):
    log_error("TELEGRAM_HASH is missing or invalid! Exiting now")
    exit(1)
...
if AUTHORIZED_CHATS:
    aid = AUTHORIZED_CHATS.replace(',', ' ').split()
    for id_ in aid:
...
if len(SUDO_USERS) != 0:
    aid = SUDO_USERS.replace(',', ' ').split()
    for id_ in aid:
        user_data[int(id_.strip())] = {'is_sudo': True}
        
BLACKLIST_USERS = environ.get('BLACKLIST_USERS', '')
if len(BLACKLIST_USERS) != 0:
    for id_ in BLACKLIST_USERS.replace(',', ' ').split():
...
WEB_PORT = int(environ.get('PORT') or BASE_URL_PORT)
ENABLE_WEB_SERVER = environ.get('ENABLE_WEB_SERVER', 'true').lower() == 'true'
...
def aria2c_init():
    for attempt in range(20):
        try:
            version = aria2.client.get_version()['version']
            log_info(f"Aria2c v{version} is ready")
            return
        except Exception as e:
            if attempt == 19:
                raise RuntimeError(f"Aria2c failed to start: {e}") from e
            sleep(0.5)
