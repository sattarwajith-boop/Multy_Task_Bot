...
class DbManger:
    ...
    def __connect(self):
        if not DATABASE_URL:
            ...
            return
        try:
            self.__conn = AsyncIOMotorClient(
                DATABASE_URL,
    ...
    async def db_load(self):
        if self.__err:
            return
        # Save non-secret bot settings. Runtime credentials belong in Koyeb
        # Secrets and must not be copied into MongoDB.
        stored_config = {
        ...
        await self.__db.settings.config.update_one(
            ...
            {
                '$set': stored_config,
                '$unset': {key: '' for key in SENSITIVE_CONFIG_KEYS},
            },
            upsert=True,
        )
        # Save Aria2c options
    ...
    async def update_config(self, dict_):
        ...
        if safe_values:
            await self.__db.settings.config.update_one(
                {'_id': bot_id}, {'$set': safe_values}, upsert=True
            )
        self.__conn.close()
    ...
    async def update_user_data(self, user_id):
        if self.__err:
            return
        data = deepcopy(user_data[user_id])
        if data.get('thumb'):
            del data['thumb']
        if data.get('rclone'):
            del data['rclone']
    ...
    async def get_pm_uids(self):
        if self.__err:
            return
        user_ids = [doc['_id'] async for doc in self.__db.pm_users[bot_id].find({})]
        self.__conn.close()
        return user_ids
